...
def change_prolog(self, **words):
    try:
        ...
        if self.selected_pocket < 0:
            print("*** M6: no tool prepared")
            self.set_errormsg("M6: no tool prepared")
            return INTERP_ERROR
        if self.cutter_comp_side:
            print("*** cannot change tools with cutter radius compensation on")
        ...
        print(self.current_tool, self.selected_tool, self.current_pocket, self.selected_pocket)
        return INTERP_OK
    except Exception as e:
        print("*** exception in change_prolog:" + e)
